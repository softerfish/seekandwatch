...
class WebhookRegistrar:
    ...
    def _construct_webhook_url(self, tunnel_url: str) -> str:
        """
        Construct webhook URL by appending path to tunnel URL.
        ...
        Returns:
            Full webhook URL
        """
        return Router.get_local_webhook_url(tunnel_url)
    ...
    def test_connection(self, timeout: int = 30) -> Tuple[bool, str]:
        ...
        try:
            ...
            response = requests.post(
                Router.get_cloud_url(self.cloud_base_url, Router.CLOUD_TEST_WEBHOOK),
                headers={
                    'X-Server-Key': self.api_key,
                    'Content-Type': 'application/json'
                },
