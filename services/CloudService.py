...
class CloudService:
    ...
    def log_webhook(event, payload, status, message=None):
        ...
        try:
            ...
            # prune old logs based on user preference
            max_logs = 100
            if settings and getattr(settings, 'max_webhook_logs', None):
                max_logs = max(10, min(1000, settings.max_webhook_logs))  # clamp between 10 and 1000
    ...
    def get_cloud_import_log(limit=20):
        """grab the local cloud import log"""
        try:
