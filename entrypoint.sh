...
if [ -d "/app/assets/custom_posters" ]; then
    ...
    if [ "$file_count" -gt 0 ]; then
        echo "Migrating $file_count custom poster(s) to /config/custom_posters/..."
        for poster_file in "/app/assets/custom_posters"/*; do
            [ -f "$poster_file" ] || continue
            fname=$(basename "$poster_file")
            if [ ! -f "/config/custom_posters/$fname" ]; then
                cp "$poster_file" "/config/custom_posters/$fname"
            fi
        done
        echo "Done migrating poster artwork."
    fi
fi
chown -R appuser:appuser /config/custom_posters
# -----------------------------------------------------
...
# 5. Start the App
# AGGRESSIVE Python bytecode cache clearing to prevent stale code issues
echo "Clearing Python bytecode cache..."
# Clear from APP_DIR
find "$APP_DIR" -type d -name __pycache__ -exec rm -rf {} + 2>/dev/null || true
find "$APP_DIR" -name "*.pyc" -delete 2>/dev/null || true
find "$APP_DIR" -name "*.pyo" -delete 2>/dev/null || true
# Also clear from /app (image location) in case it's being used
find /app -type d -name __pycache__ -exec rm -rf {} + 2>/dev/null || true
find /app -name "*.pyc" -delete 2>/dev/null || true
find /app -name "*.pyo" -delete 2>/dev/null || true
# Extra cleanup for api directory (common source of stale cache)
rm -rf "$APP_DIR/api/__pycache__" 2>/dev/null || true
