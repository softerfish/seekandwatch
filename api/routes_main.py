...
# simple in-memory cache so we don't hit TMDB on every page refresh
...
def get_public_posters():
    ...
    now = time.time()
    if _poster_cache['posters'] and (now - _poster_cache['updated_at']) < POSTER_CACHE_TTL:
        return jsonify({'posters': _poster_cache['posters']})

    try:
        # grab the first available tmdb key (just for background art, no user data involved)
        settings = Settings.query.filter(Settings.tmdb_key != None, Settings.tmdb_key != '').first()
        ...
        # grab full-size backdrops where available, fall back to posters
        urls = []
        for m in movies:
            path = m.get('backdrop_path') or m.get('poster_path')
            if path:
                urls.append(f'https://image.tmdb.org/t/p/w1280{path}')

        random.shuffle(urls)
        _poster_cache = {'posters': urls, 'updated_at': now}
        return jsonify({'posters': urls})
    except Exception:
        return jsonify({'posters': []})


@api_bp.route('/upload_artwork', methods=['POST'])
@login_required
@admin_required
def upload_artwork():
    ...
    try:
        ...
        if file and file.filename:
            ...
            try:
                ...
                current_app.logger.error(f"Artwork download error: {str(e)}")
                return _error_response(f"Failed to download image: {str(e)}")
        ...
        # Log the actual error for debugging
        current_app.logger.error(f"Artwork upload error: {str(e)}")
        import traceback
        traceback.print_exc()
...
@api_bp.route('/update_filters', methods=['POST'])
@login_required
def update_filters():
    data = request.json
...
@api_bp.route('/get_available_libraries')
@login_required
def get_available_libraries():
    """Get Plex libraries grouped by type for multi-library collection support."""
    s = current_user.settings
    try:
        plex = PlexServer(s.plex_url, s.plex_token)
...
@api_bp.route('/plex/collection/visibility', methods=['POST'])
@login_required
def set_plex_collection_visibility():
    ...
    if key_path:
        ...
        try:
            ...
            from services.CollectionService import CollectionService
            CollectionService.apply_collection_visibility(col, visible_home=visible_home, visible_library=visible_library, visible_friends=visible_friends)
...
@api_bp.route('/plex/library/sync', methods=['POST'])
@login_required
        ...
        if r.status_code == 200:
            ...
            if isinstance(data, list):
                for res in data:
                    ...
                    is_owned = res.get('owned') is True or res.get('owned') == 1
                    for c in (res.get('connections') or []):
                        ...
                        connections.append({'uri': uri, 'local': bool(local), 'label': label, 'token': res_token, 'owned': is_owned})
                        seen_uris.add(uri)
                        ...
                        if 'plex.direct' in host:
                            ...
                            if ip_uri and ip_uri not in seen_uris:
                                try:
                                    ...
                                    connections.append({'uri': ip_uri, 'local': is_private, 'label': f"{name} ({ip_host}) {label_suffix}", 'token': res_token, 'owned': is_owned})
                                    seen_uris.add(ip_uri)
                                except Exception:
                                    pass
    except requests.RequestException:
        _log_api_exception("plex_resources")
    # Sort: Owned servers first, then local IP recommended, then other local, then remote
    def _sort_key(x):
        is_owned = x.get('owned', False)
        is_ip = '[local IP]' in (x.get('label') or '')
        # Sort priority: Owned (0) > Not Owned (1)
        # Then: Recommended Local IP (0) > Other (1)
        # Then: Local (0) > Remote (1)
        return (not is_owned, not is_ip, not x['local'], x['label'])
    connections.sort(key=_sort_key)
    return user_info, connections

@api_bp.route('/plex/pin/create', methods=['POST'])
@login_required
def plex_pin_create():
    ...
    if poll.get('authToken'):
        ...
        if connections:
            ...
            if not best_server and '.' in current_host:
                ...
                for c in owned_conns:
                    if subnet in c.get('uri', ''):
                        best_server = c
                        break

            # 4. PRIORITY 3: First "[local IP] - recommended" from an owned server
            if not best_server:
                for c in owned_conns:
...
@api_bp.route('/plex/set-url', methods=['POST'])
@login_required
def plex_set_url():
...
@api_bp.route('/plex/unlink', methods=['POST'])
@login_required
def plex_unlink():
...
def health_status_api():
    """Return real-time connection status for all services (Parallel)."""
    s = current_user.settings
    results = {
    ...
    def check_plex():
        ...
        try:
            ...
            return 'plex', {'status': 'offline', 'message': 'Offline'}
        except requests.exceptions.SSLError:
            return 'plex', {'status': 'online', 'message': 'Connected (SSL Warning)'}
        except:
            return 'plex', {'status': 'offline', 'message': 'Error'}

    def check_radarr():
        if not (s.radarr_url and s.radarr_api_key): return None
        try:
    ...
    def check_sonarr():
        if not (s.sonarr_url and s.sonarr_api_key): return None
        try:
            from services.IntegrationsService import IntegrationsService
    ...
    def check_cloud():
        if not (s.cloud_enabled and s.cloud_api_key): return None
        try:
            ...
            if r.status_code == 200:
                msg = "Connected"
                if getattr(s, 'tunnel_url', None):
                    msg = "Connected (Tunnel Active)" if s.tunnel_status == 'connected' else f"Connected (Tunnel {s.tunnel_status})"
                return 'cloud', {'status': 'online', 'message': msg}
            return 'cloud', {'status': 'offline', 'message': f'Cloud Error {r.status_code}'}
        except: return 'cloud', {'status': 'offline', 'message': 'Offline'}
...
@api_bp.route('/radarr_sonarr_scanner/save', methods=['POST'])
@login_required
...
@api_bp.route('/scanner/aliases', methods=['GET'])
...
@api_bp.route('/scanner/save', methods=['POST'])
@login_required
...
@api_bp.route('/scanner/log_size', methods=['POST'])
@login_required
...
@api_bp.route('/scanner/reset', methods=['POST'])
@login_required
def reset_scanner():
...
@api_bp.route('/kometa_templates', methods=['GET'])
@login_required
...
@api_bp.route('/kometa_templates', methods=['POST'])
@login_required
...
def import_kometa_config():
    ...
    try:
        ...
        print(f"ERROR in import_kometa_config: {str(e)}")
        import traceback
        traceback.print_exc()
        return jsonify({'status': 'error', 'message': 'Import failed'}), 500

@api_bp.route('/sync_aliases', methods=['POST'])
@login_required
...
@api_bp.route('/admin/toggle_role', methods=['POST'])
...
@api_bp.route('/recovery_codes/generate', methods=['POST'])
...
@api_bp.route('/recovery_codes/use', methods=['POST'])
@rate_limit_decorator("5 per hour")
...
@api_bp.route('/media/overview')
@login_required
def get_media_overview():
    """Fetch media from Radarr and Sonarr."""
    s = current_user.settings
...
@api_bp.route('/radarr/quality-profiles', methods=['GET'])
@login_required
...
@api_bp.route('/sonarr/quality-profiles', methods=['GET'])
@login_required
...
@api_bp.route('/sonarr/add', methods=['POST'])
@login_required
def add_to_sonarr():
...
@api_bp.route('/get_artwork_path', methods=['GET'])
@login_required
@admin_required
...
@api_bp.route('/delete_artwork', methods=['POST'])
@login_required
@admin_required
def delete_artwork():
    try:
...
@api_bp.route('/radarr/movie/<int:movie_id>', methods=['GET'])
@login_required
...
@api_bp.route('/radarr/search', methods=['POST'])
@login_required
def radarr_search():
...
@api_bp.route('/radarr/refresh/<int:movie_id>', methods=['POST'])
@login_required
...
@api_bp.route('/radarr/search-scan/<int:movie_id>', methods=['POST'])
@login_required
...
@api_bp.route('/radarr/queue-check/<int:movie_id>', methods=['GET'])
@login_required
...
@api_bp.route('/sonarr/refresh/<int:series_id>', methods=['POST'])
@login_required
...
@api_bp.route('/sonarr/search-scan/<int:series_id>', methods=['POST'])
@login_required
...
@api_bp.route('/sonarr/queue-check/<int:series_id>', methods=['GET'])
@login_required
...
@api_bp.route('/sonarr/search-episode/<int:episode_id>', methods=['POST'])
@login_required
...
@api_bp.route('/sonarr/search', methods=['POST'])
@login_required
...
@api_bp.route('/sonarr/download', methods=['POST'])
@login_required
...
@api_bp.route('/calendar/episode/<int:episode_id>', methods=['GET'])
@login_required
...
@api_bp.route('/calendar/episode/<int:episode_id>/releases', methods=['GET'])
@login_required
...
# Test Runner API
@api_bp.route('/run_test', methods=['POST'])
@login_required
@admin_required
@rate_limit_decorator("5 per hour")  # prevent abuse
def run_test():
...
@api_bp.route('/run_all_tests', methods=['POST'])
@login_required
@admin_required
@rate_limit_decorator("3 per hour")  # prevent abuse
def run_all_tests():
    """run all tests with security validation"""
    from utils.secure_test_runner import SecureTestRunner
    import os
    ...
    # get all allowed tests
    all_tests = runner.get_allowed_tests()

    if not all_tests:
        return jsonify({'status': 'error', 'message': 'no tests found'}), 404

    write_log("info", "TestRunner", f"running {len(all_tests)} tests")

    # run all tests (with combined timeout)
    all_stdout = []
    all_stderr = []
    total_passed = 0
    total_failed = 0
    ...
    for test_file in all_tests:
        result = runner.run_test(test_file, timeout=30)
        if result['success']:
            total_passed += 1
            all_stdout.append(f"✓ {test_file}: PASSED")
        else:
            total_failed += 1
            all_stdout.append(f"✗ {test_file}: FAILED")

        # add test output
        if result.get('stdout'):
            all_stdout.append(result['stdout'][:500])
        if result.get('stderr'):
            all_stderr.append(f"{test_file}: {result['stderr'][:500]}")
    ...
    # add summary
    summary = f"\n\n{'='*50}\nSummary: {total_passed} passed, {total_failed} failed out of {len(all_tests)} tests"
    stdout_text += summary
...
@api_bp.route('/cloud/test', methods=['POST'])
@login_required
